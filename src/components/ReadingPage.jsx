...
const READING_LEVELS = CATEGORY_LEVELS?.comm_reading || COMM_READING_LEVELS.map((l) => l.topic);


/**
 * ReadingPage
 * Shows the first 5 Reading levels.
 * Level 1 is always unlocked; others unlock sequentially (1 star required).
 *
 * Props:
 *   onBack()                          – return to Communication Skills page
 *   unlockedLevels  (number[])        – array of unlocked level numbers for reading
        ...
        <div style={{ padding: '20px 0 40px 0' }}>
          <div className="category-container">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {READING_LEVELS.map((topic, index) => {
                const levelNum = index + 1;
                const levelId = `level-comm_reading-${levelNum}`;
